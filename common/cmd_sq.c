...
uint32_t update_kernel_address_noapi(void)
{
	...
	if (ret != 0) {
		printf("Error: 'sf read' command failed with return code %d\n", ret);
		return 0;
	}
	...
	if (!baseaddr_str) {
		printf("Error: Environment variable 'baseaddr' not set.\n");
		return 0;
	}
	...
	if (offset_found < 0) {
		printf("Error: Kernel magic (0x%x) not found in flash block\n", KERNEL_MAGIC_NUMBER);
		return 0;
	}
	...
	kernel_flash_addr = KERNEL_FLASH_OFFSET + offset_found;
	{
		...
		printf("Kernel start address found: %s\n", kern_addr_str);
	}
...
uint32_t update_rootfs_address_noapi(void)
{
	...
	if (!kern_addr_str) {
		printf("Error: Environment variable 'kern_addr' not set.\n");
		return 0;
	}
	...
	if (offset_found < 0) {
		printf("Error: rootfs magic (0x%x) not found in flash block\n", SQUASHFS_MAGIC);
		return 0;
	}
	...
	rootfs_flash_addr = kernel_flash_addr + 0x140000 + offset_found;
	{
		...
		printf("rootfs start address found: %s\n", rootfs_addr_str);
	}
...
static void compute_kernel_partition_size(void)
{
	...
	if (!kern_addr_str || !rootfs_str) {
		printf("Error: kern_addr or rootfs not set in env\n");
		return;
	}
	...
	if (rootfs < kern_addr) {
		printf("Error: rootfs (0x%lx) is less than kern_addr (0x%lx)\n", rootfs, kern_addr);
		return;
	}
	...
	printf("Computed kernel partition size (hex): %s\n", buf);
	...
	printf("Computed kernel partition size (decimal): %s\n", buf_dec);
}
